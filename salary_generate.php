    ...
    <div class="card col-md-12 " >
        ...
        <tbody>
            ...
            if(isset($_POST['month'])   && isset($_POST['year'])) {
              $sMonth  = $_POST['month'];
              $sYear = $_POST['year'];
              $query = "SELECT a.month, a.year, a.ot_hours,a.employee_code,a.employee_name,a.pay_days,a.absent_days,e.employee_salary,a.ot_hours FROM attendance a JOIN employees e WHERE a.month='$sMonth' AND a.year='$sYear' AND a.employee_code = e.employee_code ORDER BY a.employee_name";
              ...
              if ($result->num_rows > 0) {
                  while ($row = $result->fetch_assoc()) {
                      $extra_days = 0;
                      $totalOT = 0;
                      $totalOTHr = $row['ot_hours'];
                      $absent_days=   $row['absent_days'];
                      if($absent_days != "0") { $extra_days = 1 ; }
                      $totalMonthDays = $row['pay_days'] + $row['absent_days']; 
                      $totalSunday = countSundays($row['month'], $row['year']);
                      $perDayPay = $row['employee_salary'] / ($totalMonthDays );
                      $salary = ($perDayPay) * ( $row['pay_days'] + $extra_days );
                      $perHoursPay = $perDayPay / 9;
                     
                      if($totalOTHr > 0.3 ) {  $totalOT = $perHoursPay * $totalOTHr; }
                      $finalSalary = $salary + $totalOT;
                      $sundayPay = $perDayPay * $totalSunday;
                      $sundayCancelSalary = $finalSalary - $sundayPay;
                    
        

          
                      if($row['ot_hours'] > 0.3 ) {  $totalOT = $perHoursPay * $row['ot_hours']; }
                      ...
                      echo "<td>" . $row['absent_days'] . " Days </td>";
                     
                      echo "<td > ₹ " . $row['employee_salary'] . "</td>";
                      echo "<td>" . $row['ot_hours'] . " hr </td>";
                      echo "<td > ₹ ". round($totalOT)."</td>";
                      echo "<td > ₹ ". round($salary)."</td>";
                      echo "<td style='color:green'> ₹ ". round($finalSalary)."</td>";
                      if($absent_days > 4 ) {
                        echo "<td style='color:green'> ₹ ". round($sundayCancelSalary)."</td>";
